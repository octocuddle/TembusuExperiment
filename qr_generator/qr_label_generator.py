...
def create_pdf(rows):
    ...
    for _, row in rows.iterrows():
        uid = row.get("copy_id", row.get("book_id", "unknown"))
        title = row["title"]
...
1. **Upload** your CSV file with book information  
2. **Select** the rows you want to print using the checkboxes  
3. **Download** the PDF and print the labels  
4. Stick the QR labels on the corresponding books
""")
...
## 1. Upload your CSV file with book information  
""")
uploaded_file = st.file_uploader("Upload CSV with book data", type=["csv"])
...
if uploaded_file:
    ...
    edited_df = st.data_editor(
        ...
        disabled=["copy_id", "title", "call_number", "isbn"]  # make other columns read-only
        )
